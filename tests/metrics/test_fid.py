...
def test_calculate_fid(request):
    ...
    left_img_1 = Image.open(os.path.join(request.fspath.dirname, FIXTURES_DIR, "left_img_1.png"))
    right_img_1 = Image.open(os.path.join(request.fspath.dirname, FIXTURES_DIR, "right_img_1.png"))
    left_img_2 = Image.open(os.path.join(request.fspath.dirname, FIXTURES_DIR, "left_img_2.png"))
    img_unrelated = Image.open(os.path.join(request.fspath.dirname, FIXTURES_DIR, "img_unrelated.jpg"))
    ...
    fid_value_diff_view = calculate_fid(preds, truths, batch_size=1, dims=2048, device='cpu')

    # compare images that are unrelated
    truths = transforms.ToTensor()(img_unrelated).unsqueeze_(0)
    fid_value_unrelated = calculate_fid(preds, truths, batch_size=1, dims=2048, device='cpu')
